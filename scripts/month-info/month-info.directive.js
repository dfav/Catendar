;(function () {
  ...
  function MonthInfo () {
    ...
    function MonthInfoController (catendarService) {
      ...
      // Constructor
      function CalendarDay (number, isThisMonth, isToday) {
        // Assign default values in case params were not passed
        number = typeof number === 'undefined' ? 0 : number
        isThisMonth = typeof isThisMonth === 'undefined' ? false : isThisMonth
        isToday = typeof isToday === 'undefined' ? false : isToday
      ...
      function Load () {
        catendarService.getTodaysDate()
          .then(function (response) {
            ...
            for (var i = lastDayPreviousMonth - monthStart + 1; i <= lastDayPreviousMonth; i++) {
              ctrl.calendarDays.push(new CalendarDay(i))
            }

            // Next we add all of the days in this month
            for (var i = 1; i <= lastDayThisMonth; i++) {
            ...
            for (var i = 1, l = ctrl.daysOfWeek.length - (ctrl.calendarDays.length % ctrl.daysOfWeek.length); i <= l; i++) {
              ctrl.calendarDays.push(new CalendarDay(i))
            }
          }, function (error) {
